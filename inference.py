...
def get_bbox(img):
    ...
    preds = next(iter(model.predict(img_rgb, conf=args['conf'])._images_prediction_lst))
...
class_names = next(iter(model.predict(np.zeros((1,1,3)), conf=args['conf'])._images_prediction_lst)).class_names
